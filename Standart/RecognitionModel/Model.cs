            ...
            for (int i = 0; i < Environment.ProcessorCount; i++)
            {
                threads[i] = new Thread(()=>
                {
                    ...
                    while(true)
                    {
                        ...
                        ImageNum=Interlocked.Increment(ref processed);
                        if(ImageNum>= Images.Count() || cts.Token.IsCancellationRequested)
                            break;
                        else
                        {
                            (int class_num, float prob) = ProcessFile(Images[ImageNum]);
                            OutputEvent?.Invoke(this, new OutputHandlerEventArgs(Images[ImageNum], class_num, prob));
